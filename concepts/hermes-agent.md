...
后续使用文章补充了一个很实操的视角：Hermes 装好以后，真正决定它像不像“你的长期助理”的，往往不是安装命令，而是后续的身份、记忆、联网与入口配置。

- **身份与个性**：`~/.hermes/SOUL.md` 是全局身份锚点，`/personality` 和 `/skin` 则是更轻量的会话或视觉层配置
- **记忆扩展**：除了内置三层记忆，还能外挂 Holographic、ByteRover、Honcho、Mem0、OpenViking 等外部 provider
- **Session Search**：和长期记忆互补；无 query 时偏最近会话浏览，有 query 时走 FTS5 检索 + LLM 摘要
- **联网与浏览器能力**：可配置 Firecrawl / Exa / Tavily 等 `web_search`，并借助 Camofox 这类浏览器环境处理反爬页面
- **远程入口**：Gateway 可接到微信等消息入口，让 Hermes 不再局限于本地终端
- **MCP 集成**：从 `~/.hermes/config.yaml` 读取 stdio / HTTP 两类 MCP server，作为“后装能力面”持续扩展

## 关联页面

- [[products/everything-claude-code]] — Anthropic 黑客松冠军的 Claude Code 配置体系，与 Hermes Agent 有相似的"把方法论变成可复用结构"的设计思想
- [[concepts/agent-memory]] — Agent Memory 的上位问题框架，Hermes 的分层记忆与技能系统是其中一种实现
- [[concepts/openclaw]]
